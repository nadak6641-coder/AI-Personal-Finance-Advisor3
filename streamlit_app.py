...
Also includes a personal expense & income tracker (SQLite-backed) — see
08_expense_tracker.py. Every user now signs in with a username/password
(one shared database, but each account only ever sees its own records).
Financial log entries can be typed directly into the chat box (e.g.
"صرفت 200 جنيه أكل") or entered through the dedicated form in the
"Expense Tracker" tab.
...
# ---------------------------------------------------------------------------
# Login gate — nothing below this runs until the user is signed in.
# Each account only ever sees the income/expense rows tied to its own
# user_id, so one person's data is never visible to anyone else.
# ---------------------------------------------------------------------------
...
if st.session_state.user_id is None:
    ...
    st.caption("سجّلي دخول أو اعملي حساب جديد عشان تبدئي.")

    login_tab, register_tab = st.tabs(["🔑 تسجيل الدخول", "🆕 حساب جديد"])

    with login_tab:
        with st.form("login_form"):
            login_username = st.text_input("اسم المستخدم", key="login_username")
            login_password = st.text_input("الباسورد", type="password", key="login_password")
            login_submitted = st.form_submit_button("دخول", type="primary")

        if login_submitted:
            found_user_id = expense_tracker.authenticate_user(login_username, login_password)
    ...
    with register_tab:
        with st.form("register_form"):
            ...
            new_password = st.text_input("باسورد جديد (٨ حروف على الأقل)", type="password", key="register_password")
            register_submitted = st.form_submit_button("إنشاء حساب", type="primary")

        if register_submitted:
            try:
                expense_tracker.register_user(new_username, new_password)
                st.success("✅ اتعمل الحساب بنجاح! دلوقتي سجّلي دخول من التاب التاني.")
...
# ---------------------------------------------------------------------------
# Signed in — everything below is scoped to st.session_state.user_id
# ---------------------------------------------------------------------------
with st.sidebar:
...
with chat_tab:
    ...
    if st.button("Ask", type="primary") and query.strip():
        ...
        log_result = expense_tracker.parse_and_log_message(st.session_state.user_id, query)
        ...
        if log_result is not None:
            ...
            summary = expense_tracker.get_monthly_summary(st.session_state.user_id)
...
with tracker_tab:
    ...
    if submitted and amount > 0:
        if entry_kind == "مصروف":
            expense_tracker.add_expense(
                st.session_state.user_id, amount, category=category, note=note,
                entry_date=entry_date.isoformat(),
            )
            st.success(f"✅ تم تسجيل مصروف {amount:.0f} جنيه تحت «{category}».")
        else:
            expense_tracker.add_income(
                st.session_state.user_id, amount, source=source,
                entry_date=entry_date.isoformat(),
            )
    ...
    trend = expense_tracker.get_monthly_trend(st.session_state.user_id, 6)
    ...
    recent_expenses = expense_tracker.get_all_expenses(st.session_state.user_id, limit=10)
    recent_income = expense_tracker.get_all_income(st.session_state.user_id, limit=10)
